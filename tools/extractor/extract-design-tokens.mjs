...
export async function extractDesignTokens(targetUrl, outputDir = "./extracted-tokens") {
  console.log(`\n======================================================`);
  ...
  try {
    ...
    const extracted = await page.evaluate(() => {
      const colors = { backgrounds: {}, text: {}, borders: {} };
      const typography = { families: {}, headings: {}, body: {} };
      ...
      elements.forEach((el) => {
        ...
        if (textContent && textContent.length > 0) {
          ...
          if (/^h[1-6]$/.test(tag)) {
            if (!typography.headings[tag]) typography.headings[tag] = typeSpec;
          } else if (tag === "p" || tag === "span" || tag === "div") {
            const key = `${style.fontSize}-${style.fontWeight}`;
            if (!typography.body[key]) typography.body[key] = { ...typeSpec, sample: textContent.slice(0, 30) };
          }
        }
    ...
    const processedTokens = {
      meta: { sourceUrl: targetUrl, extractedAt: new Date().toISOString() },
      colors: {
